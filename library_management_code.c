...
#define FILE_NAME  "bookmaster.bin"
...
{
    system("cls");//pass the commands that can be executed in the terminal,returns the command after it has been completed.
    printf("\t\t\t###########################################################################");
    printf("\n\t\t\t############                                                   ############");
...
void welcomeMessage()
{
    headMessage("bookmasterlibrary.com");
    printf("\n\n\n\n\n");
    printf("\n\t\t\t  **-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**\n");
    printf("\n\t\t\t        =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
    printf("\n\t\t\t        =                 WELCOME                   =");
...
void withdrawBooks()
{
    ...
    FILE *tmpFp = NULL;
    int status = 0;
    headMessage("Withdrawn Books Details");
    fp = fopen(FILE_NAME,"rb");
    if(fp == NULL)
    {
        printf("File is not opened\n");
    ...
    (found)? printf("\n\t\t\tRecord withdrawn successfully....."):printf("\n\t\t\t unable to withdraw ");
    fclose(fp);
    fclose(tmpFp);
    remove(FILE_NAME);
...
void login()
{
    unsigned char userName[MAX_SIZE_USER_NAME] = {0};
    unsigned char password[MAX_SIZE_PASSWORD] = {0};
    int L=0;//count to check no of login tries
    sFileHeader fileHeaderInfo = {0};
    FILE *fp = NULL;
    headMessage("Login");
    fp = fopen(FILE_NAME,"rb");
...
void init()
{
    FILE *fp = NULL;
    int status = 0;
    const char defaultUsername[] ="BookMasters\n";
